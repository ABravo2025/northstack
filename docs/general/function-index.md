...
| `billing.ts` (Billing Integration, Etapa E) | getSubscription, startCheckout, changeSubscriptionPlan (post-billing, distinto de `updateTenantPlan` de arriba que es la elección pre-billing durante trial), cancelSubscription, resumeSubscription, getInvoiceDocumentUrl(token, invoiceId) (2026-08-19, Dodo Payments-only, se pide fresca en cada click — `BillingPage.tsx` la usa para el link "View invoice" por fila de Invoice; sin `disposition?` desde 2026-09-13, Dodo no tiene ese concepto) |
