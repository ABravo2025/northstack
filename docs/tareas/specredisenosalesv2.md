...
### 3.8 Automatizaciones — completo (2026-08-25, en `staging`)

- [x] **Construido — Unidad 8**: Schema — `Pipeline.assignmentMode`, enum `'round_robin' | 'account_owner'`, nullable (null = sin auto-asignación), configurable por Pipeline en el modal de Edit. `Pipeline.stalledThresholdDays` int nullable (null = recordatorio desactivado). `Pipeline.lastAssignedUserId` (cursor de rotación, escrito con `$executeRaw` para no pisar `Pipeline.updatedAt` en cada creación — ver más abajo). `Opportunity.ownerId` pasó a nullable (`String?`) — consecuencia necesaria de la degradación prolija (ver siguiente punto), no una decisión aparte.
- [x] **Resuelto 2026-08-24 — quiénes rotan en el round-robin**: en vez de inferir el universo desde el rol, el owner/admin lo define a mano por Pipeline. Schema `PipelineAssignmentUser` (`id`, `tenantId`, `pipelineId` FK, `userId` FK, `@@unique([pipelineId, userId])`) — join table simple, mismo patrón que `EmployeeTimeOffPolicy`. `lastAssignedUserId` rota solo entre los `User` listados ahí (no todos los del tenant). Vacío o sin nadie elegible → la Opportunity queda sin owner (degradación prolija, mismo criterio que `mailerConfigured()`), nunca un error.
- [x] **Refinado 2026-08-25 — elegibilidad del round-robin**: un participante se salta si no es actualmente un Employee activo (`Employee.statusId` == el `StatusDefinition` default de `employee` para el tenant, vía `getDefaultStatusId` — nunca comparar por nombre) **y** su `User.status === 'active'`. Un User sin Employee vinculado queda permanentemente inelegible (documentado en la UI, no un caso especial en la lógica).
- [x] **Refinado 2026-08-25 — alta masiva de participantes por Departamento**: el picker de participantes (checklist de Users) se complementa con un multi-select de Departamento — resuelve "quién tiene un Employee en estos departamentos hoy" e inserta filas `PipelineAssignmentUser` normales, una sola vez. **No es un vínculo vivo**: no hay schema nuevo departamento↔pipeline, no se re-resuelve más adelante, y un participante agregado así es indistinguible de uno agregado a mano.
- [x] **Resuelto 2026-08-25 — disparador del round-robin**: corre en la creación de la Opportunity. Un `ownerId` explícito en el request siempre se respeta tal cual (round-robin nunca lo consulta). Si se omite y el Pipeline es `round_robin`, se resuelve el siguiente participante elegible. Siempre reasignable a mano después.
- [x] **Ajustado 2026-08-25 (ampliado respecto a la ronda anterior)** — Regla `account_owner`: aplica tanto en la creación directa en un pipeline `type: 'account'` como al mover una Opportunity hacia uno (3.6). Si `company.accountOwnerId` está seteado, **sobreescribe** `Opportunity.ownerId` (a menos que el mismo request traiga un `ownerId` explícito, que siempre gana) — es un traspaso deliberado al manager de la cuenta, no un "solo si está vacío". Si la Company no tiene `accountOwnerId`, fallback a round-robin (ahí sí "solo si está vacío") sobre los participantes de ese mismo Pipeline.
- [x] **Construido — Unidad 8**: primer cron real del proyecto — `runStalledOpportunityReminders` (`src/modules/crm/stalledOpportunityService.ts`), disparado por Vercel Cron (`GET /api/internal/opportunities/stalled-reminders/run`, `vercel.json`) una vez al día. Por Pipeline con `stalledThresholdDays` seteado, notifica + emaila al owner de las Opportunities `open` estancadas. Deduplicación "una vez por episodio de estancamiento" sin schema nuevo: se compara el último `Notification` de ese tipo/entidad/usuario contra `stageHistory[0].enteredAt` de la Opportunity — un cambio de stage siempre escribe una fila nueva, así que invalida sola la notificación anterior y permite una nueva tras el próximo estancamiento. Excluye tenants `suspended`/`cancelled` (a diferencia de los otros dos crons, este le manda mail a usuarios reales). Opportunities sin owner se cuentan y se saltan, nunca se le mandan a otro.
- [x] **Construido — Unidad 8**: notificación de cambio de stage — sincrónica, en el mismo `updateOpportunity` que ya escribe `OpportunityStageHistory`. Notifica al owner post-cambio, nunca al propio actor sobre su cambio. El email sale en **cada** cambio de stage (decisión explícita de Alejandro 2026-08-25) — **pendiente de backlog**: Alejandro pidió dejar anotado que esto debería volverse configurable más adelante vía una sección real de preferencias de notificación en el perfil/Settings del usuario, para no "acosarlo"; no se construye en esta unidad.
- [x] **Fuera de alcance, a propósito**: las Opportunities creadas por Public Form mantienen su fallback hardcodeado al owner del tenant (sin cambios). El default `accountOwnerId || currentUserId` de `CompanyDetailModal.tsx` se mantiene (ahora ajustado a `accountOwnerId || (autoAssigns ? undefined : currentUserId)` para no pisar la automatización del Pipeline elegido).
- [x] **Verificado 2026-08-25** de punta a punta contra `staging` real con dos tenants descartables (uno para aislamiento), 40/40 chequeos en verde: rotación en orden + persistencia del cursor sin tocar `updatedAt`; participantes inactivos (Employee inactivo o User inactivo) salteados; degradación a `ownerId: null` tanto con lista vacía como con todos inelegibles; `ownerId` explícito nunca sobreescrito y nunca avanza el cursor; `assignmentMode: null` sigue exigiendo `ownerId` (400 a nivel ruta); `account_owner` sobreescribe en creación y en movimiento cuando `accountOwnerId` está seteado, y cae a round-robin (fill-only-if-empty en movimiento) cuando no lo está; notificación de cambio de stage se dispara para un no-actor y no para un self-change; el cron de estancados crea, dedupea en una re-corrida inmediata, y re-notifica tras un nuevo estancamiento en otro stage; el cron rechaza sin auth y con secret incorrecto (401) y acepta el `CRON_SECRET` correcto (200); alta masiva por departamento resuelve el set correcto y no rompe en un re-alta superpuesto; aislamiento entre tenants confirmado (round-robin, listado de participantes, intento de asignación cruzada). `npm run build`/`npm test` (91/91) backend y `npm run build` frontend en verde durante todo el desarrollo. Script descartable, corrido y borrado (`scripts/_verify-unit8-automations.ts`, nunca commiteado).
- [x] **Fix de descubribilidad 2026-08-25** (hallazgo del usuario al revisar staging): la sección "Automations" vivía solo en el modal de Edit — un usuario nunca se enteraría de que la feature existe salvo que se le ocurriera editar un pipeline ya creado. Movida también al modal de "New Pipeline" (antes de Stages en ambos modales, por pedido explícito de ubicación), como estado en borrador que se aplica recién después de `createPipeline` (mismo patrón que las stages en borrador). De paso: los checklists de participantes y de alta por Departamento pasaron de checkboxes siempre visibles a un dropdown multi-select nuevo (`MultiSelectDropdown.tsx`, no existía nada así en el proyecto — construido sobre `Popover.tsx`), por pedido explícito del usuario ("mas armoniosa", "lista desplegable con multi select").
- [x] **Refinamiento 2026-08-25 — notificación por stage, no solo por Pipeline**: `PipelineStageDefinition.notifyOwnerOnEnter` (boolean, default `true`) — columna nueva en la tabla de Stages (Create y Edit) para desactivar la notificación/email de cambio de stage puntualmente en un stage ruidoso/poco relevante (ej. un "New" de entrada), sin tener que desactivar el email para todo el Pipeline. `updateOpportunity` ahora chequea el flag del stage de **destino** antes de notificar/emailear, además del chequeo existente de "nunca al propio actor". Default `true` preserva el comportamiento de "notifica en cada cambio" hasta que un tenant lo desactive puntualmente.
- [x] **Fix de UX 2026-08-26** (hallazgo del usuario al revisar staging): mostrar el picker individual de Users y el de alta-por-Departamento **a la vez**, ambos como "opcionales", leía como un error — daba la impresión de dos mecanismos compitiendo en vez de una sola forma clara de armar la lista. Cambiado a una elección explícita y obligatoria (`<select>` sin opción en blanco, default "por usuario"): "Add participants by user" vs. "Add participants by department" — solo uno de los dos pickers se muestra a la vez, y cambiar de opción limpia el borrador del otro. La mecánica de fondo no cambió (`PipelineAssignmentUser` sigue siendo una lista plana, `assignUsersByDepartments` sigue siendo la misma resolución de una sola vez, no un vínculo vivo) — esto es puramente una reorganización de cómo se presenta, no un cambio de modelo de datos. Tras agregar por Departamento en modo Edit, vuelve automáticamente a "por usuario" para mostrar la lista ya combinada. Verificado visualmente con Playwright contra un tenant descartable (con 2 departamentos reales) en ambos modales.
- [x] **Rediseño de UX 2026-08-26, superó al fix anterior** (feedback directo del usuario): el `<select>` anidado de "Add participants by user/department" seguía leyendo como dos pasos separados. Colapsado en **un solo select** de "Owner auto-assignment" con 3-4 opciones directas: `Off`, `Round robin — by user`, `Round robin — by department`, y (solo para `type: 'account'`) `Account owner`. `Pipeline.assignmentMode` en la base no cambió (sigue siendo `round_robin | account_owner | null` — la distinción "by user" vs. "by department" es pura UI, nunca se persiste, ya que el resultado final es la misma lista plana de `PipelineAssignmentUser` sin importar qué picker se usó para armarla). Al reabrir Edit de un pipeline `round_robin` existente, el select muestra "by user" por default (con la lista ya combinada), sin importar cómo se armó originalmente. **Nuevo — campo obligatorio**: en el modal de creación, elegir cualquiera de los dos `round_robin` sin seleccionar ningún participante (ni por usuario ni por departamento) bloquea el Save con un toast de error, mismo criterio que Pipeline name — `account_owner` queda exento (se degrada solo, no lo necesita). En modo Edit no hay bloqueo equivalente (no hay un único "Save" que interceptar), se mantiene la degradación prolija ya decidida para pipelines existentes.
- [x] **Nuevo 2026-08-26 — stages por default al crear**: el modal de "New Pipeline" arranca con 3 filas de stage precargadas (`Lead` / open / 50%, `Won` / won / 100%, `Lost` / lost / 0%) en vez de una fila en blanco — el usuario puede renombrarlas, cambiarles el outcome, borrarlas o agregar más, exactamente igual que antes. No afecta el seed de `seedDefaultPipelines` (los 2 pipelines default de un tenant nuevo en el registro), que es un código completamente distinto y no se tocó.
- [x] **Verificado 2026-08-26** de punta a punta con Playwright contra un tenant descartable (con 2 departamentos y un pipeline `account`): las 3 opciones del select para un pipeline `lead` (sin `account_owner`) y las 4 para uno `account`; "by user" y "by department" muestran exactamente un picker cada uno; Save bloqueado con toast cuando `round_robin` no tiene ningún participante; Save exitoso tras elegir un departamento; `account_owner` muestra la sección de participantes sin el asterisco de obligatorio y con la copy de fallback correcta; los 3 stages default aparecen con sus valores correctos. Tenant descartable limpiado, servidores de dev y scripts temporales borrados al terminar.
