...
**Tier 3 — Rediseño de Clients** (Company / Contact / Opportunity, pipeline de ventas) — **10 de 11 piezas completas, en `staging` (2026-07-27)**. Falta solo correr la migración de datos existentes contra producción — bloqueado en revisión del usuario, ver el ítem de migración más abajo y `docs/tareas/semana-2026-07-21.md` para el detalle unidad por unidad.
...
- [ ] **Rediseño de Clients — separar en Company / Contact / Opportunity (backlog, dirección confirmada por el usuario 2026-07-21, sin spec técnico todavía):** no es un agregado de campos, es un rediseño real del módulo — del mismo tamaño que el sistema de Time Off en su momento. Hoy `Client` es una sola entidad plana (firstName/lastName/email/`company` como texto libre/status). Forma conceptual confirmada, sin detalle técnico:
  ...
  - **Navegación — confirmado por el usuario 2026-07-24: 3 módulos separados en el sidebar** (Companies, Contacts, Opportunities), no un solo módulo con tabs adentro (se evaluaron las 2 opciones — el usuario fue explícito: "tienen que ser módulos distintos, ya que van por lados distintos y funciones distintas"). Nombre del módulo de Company: **"Companies"** (lenguaje plano, evita jerga tipo "Accounts").
  - **Impacto en lo ya construido, a definir en la sesión de spec dedicada:**
    - Migración real de los `Client` existentes en producción (cada `company` de texto repetido se convierte en un registro `Company`, cada `Client` pasa a ser `Contact`) — mismo tipo de migración de datos reales que el rename PTO→Time Off, no un push aditivo simple. Ahora se puede ensayar primero en el ambiente de `staging` antes de tocar producción.
  ...
  - Explícitamente **sin empezar** — queda para una sesión de spec dedicada más adelante, mismo criterio pieza-por-pieza que Time Off/Payments/Integraciones. Decisiones abiertas restantes: Kanban por stage (solo Opportunity, o también Company por lifecycle), destino de Custom Fields/Public Forms, visibilidad del account owner de Company, y si el monto de Opportunity necesita moneda explícita desde el arranque.
...
- [x] **Bug real reportado por el usuario (2026-07-26): ancho/visibilidad/orden de columnas de Employees se compartían entre todas las Views, no por View individual** — el usuario lo notó probando en vivo ("el filtro de columnas aplica a todas las vistas, no a la vista puntual donde se aplicó, es un error de lógica"). Causa real: `useResizableColumns`/`useColumnVisibility`/`useColumnOrder` (agregados en las rondas de 2026-07-22/23 de arriba) usaban una sola clave de `localStorage` por tabla (ej. `northstack:hiddenColumns:employee`), sin distinguir la View activa — ocultar una columna en "All Employees" la ocultaba también en cualquier View nueva. Corregido en `EmployeesPage.tsx` armando la clave con el `activeViewId` (`northstack:hiddenColumns:employee:${activeViewId ?? 'default'}`, mismo criterio para los otros 2 hooks) — cada View (incluida la "All Employees" implícita) tiene su propio balde de ancho/visibilidad/orden. **Gotcha real al implementarlo**: los 3 hooks solo leían `localStorage` una vez al montar (`useState(() => ...)`); cambiar la `storageKey` en caliente al cambiar de View no alcanzaba solo, y un `useEffect` naive para releer en caliente competía en carrera con el efecto que persiste — en el render donde cambia la View, el efecto de "persistir" corría con el valor de la View *vieja* y lo escribía en la clave de la View *nueva*, pisando lo que ya hubiera guardado ahí. Resuelto ajustando el estado durante el render (no en un efecto) cuando la clave cambia, patrón recomendado por React para este caso exacto. **Alcance: solo Employees** — Clients no se tocó (está por rediseñarse a Company/Contact/Opportunity, mismo motivo que las tareas de reemplicación de UI pendientes más abajo); Company Users nunca tuvo Views, no aplica. Verificado con Playwright contra un tenant de prueba real: ocultar "Personal Email" + agrandar "Name" en "All Employees", crear una View nueva → la columna vuelve a verse y el ancho vuelve al default en la View nueva; al volver a "All Employees" el ocultamiento/ancho siguen ahí como se dejaron. `npm run build` (frontend) verde, sin errores de consola.
...
- [x] **Scrollbar propia (horizontal) para las tablas tipo planilla**: spec cerrado y mockeado en el Artifact "Northstack — Scrollbars de planilla" (2026-07-22). Implementado 2026-07-24/25 en `HorizontalScrollbar.tsx` (nuevo, genérico — recibe un `targetRef` y no depende de qué tabla lo use), track gris/thumb en colores de marca, proporcional al contenido (`clientWidth/scrollWidth`), oculto si no hace falta scroll, sincroniza en ambas direcciones (drag del thumb ↔ scroll nativo), click en el track salta a esa posición. Scrollbar nativa oculta con `scrollbar-width: none` + `::-webkit-scrollbar{display:none}` sobre `.full-table-wrap`, sin librería nueva. **Alcance real: solo horizontal** (el vertical no aplica — estas tablas son a pantalla completa y scrollean con la página, no dentro de un contenedor de alto fijo). **Aplicado solo en Employees** — Clients/Company Users siguen con la scrollbar nativa, ver ítem de replicación más abajo.
  - [x] **Scroll horizontal con Shift+rueda**: implementado en el mismo componente (`handleWheel`, chequea `e.shiftKey`, `preventDefault` solo en ese caso). Ctrl+rueda descartado a propósito (ya tomado por el zoom nativo del navegador).
- [x] **Referencia de diseño ClickUp para las tablas — 6 tareas, spec cerrado y mockeado en el Artifact "Northstack — Tablas full-screen + panel Overview" (2026-07-23)**. Implementadas 2026-07-24/25, verificadas en código real:
  - [x] **Sacar el ancho fijo de `AppLayout.tsx` para pantallas de tabla**: `AppLayout.tsx` ya no envuelve en `.container` — cada página decide (`EmployeesPage.tsx`/`ClientsPage.tsx` usan `.page-full`, sin restricción de ancho; `WorkspaceSettingsLayout.tsx` mantiene `.container` centrado).
  - [x] **Vista "List" (agrupada) — tercer tipo de vista, junto a Grid y Kanban**: `SavedViewType` ganó el valor `list` (`prisma/schema.prisma`, `savedViewService.ts`, `ViewsBar.tsx`, `api.ts`), mismo `groupByField` que Kanban. Renderizada como secciones colapsables (`listSections` en `EmployeesPage.tsx`) con pill de color + contador, click colapsa/expande. **Implementada solo en Employees** — `ClientsPage.tsx` no tiene ninguna rama para `viewType === 'list'` todavía (si un usuario crea una vista 'list' en Clients hoy, cae a la tabla plana de grid sin agrupar y sin FilterBar/ColumnVisibilityMenu — bug real encontrado en code review 2026-07-25, sin arreglar aún, ver ítem de replicación).
  - [x] **Tipografía de `.full-table` más compacta, con jerarquía real entre columnas**: `.name-cell` (`text-base font-semibold`) vs. resto de celdas; headers sin mayúsculas (`.full-table th`: `text-xs font-medium text-gray-500`, sin `uppercase`/`tracking-wide`); `.full-table td` con `py-4`. Dark mode se mantuvo navy-tinteado (`gray-950`), no negro puro, como estaba decidido.
  - [x] **Panel lateral izquierdo tipo "push" — click en el nombre abre un Overview**: `EmployeeOverviewPanel.tsx` (nuevo) + `.table-panel-row`/`.table-panel-main` (flex sibling de la tabla, no overlay — angosta la tabla en vez de flotar encima). Tabs Overview/Notes/Activity (Notes/Activity placeholder sin funcionalidad, como estaba planeado). **Implementado solo en Employees, y no es genérico todavía** — si Clients lo necesita hay que extraer el layout compartido (avatar+tabs+campos), no copiar/pegar el componente entero (ver directiva del proyecto sobre este mismo panel).
  - [x] **Sacar el botón primario "Add X" del toolbar — reemplazado por fila/card fantasma siempre visible**: implementado en Grid/List/Kanban de Employees (`ghostAddRow`, `kanban-ghost-card` vía el nuevo prop `renderColumnFooter` de `KanbanBoard.tsx`). **Bug real encontrado en code review 2026-07-25, corregido el mismo día**: como la fila fantasma vive solo dentro de la tabla renderizada, un owner/admin que filtraba a 0 resultados o abría una vista Kanban/List con `groupByField` roto se quedaba sin ninguna forma de agregar un empleado (antes el botón del toolbar era incondicional). Corregido con un botón de fallback en el toolbar (`showAddFallback` en `EmployeesPage.tsx`), visible solo en esos 2 casos puntuales — el caso normal sigue siendo solo la fila fantasma, sin el botón, tal como se aprobó.
  - [x] **Hover de fila completo y visible**: `.full-table tbody tr:hover td` (por celda, no `tr:hover` solo — con columnas congeladas/`table-layout:fixed` el hover a nivel `tr` no pinta parejo en todos los navegadores).
  - [ ] **Replicar el bloque completo a Clients y Company Users** — el propio ítem original ya pedía esto ("es el estándar a replicar... no es exclusivo de Employees"), sigue sin empezar. Antes de copiar/pegar: extraer lo genérico primero — `HorizontalScrollbar.tsx` ya es reusable tal cual (recibe un `ref`); `EmployeeOverviewPanel.tsx` y la lógica de agrupamiento de `listSections` (que ya duplica lo que `KanbanBoard.tsx` hace internamente) no lo son, conviene generalizarlos antes de la tercera copia. **Nota de alcance para Company Users**: esa página nunca tuvo `SavedView`/Kanban (decisión explícita — "no tiene custom fields ni necesita vistas guardadas"), así que ahí "List/Kanban" no aplica; lo replicable es ghost row, scrollbar, hover, tipografía y un Overview panel de usuario.
