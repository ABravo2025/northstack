...
- **Payroll module (V1)** — complete (15/15 spec units), pushed to `staging` only (`docs/tareas-desarrollo.md`, Tier 3.5; full unit-by-unit detail in `docs/tareas/semana-2026-07-29.md`; data model in `docs/database-schema.md`, group 7): pay-frequency catalog, versioned per-employee compensation, runs with automatic pre-load by frequency, adjustments, hourly time entry, inactive-employee warning, run confirmation, off-cycle payments, a unified timeline, and a payslip PDF preview (new dependency `pdfkit`). Deliberately out of scope for V1: derived cost metrics, automatic pay-date calculation, and custom permissions (owner-only for now). Distinct from Payments (billing the tenant's own Clients/Companies, not paying Employees), which remains unstarted. Not in production yet — pending the user's own review in staging.
