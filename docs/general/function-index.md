...
- **runStripeEventPolling()** (Unit 4, rediseñada 2026-08-28 — reemplaza el webhook manual entero, ver QA-50 en `Tareas-QA.md`) — cron de 2x/día (`src/routes/internal.ts`): por cada `StripeConnection` activa, trae eventos nuevos vía `listEvents` desde `lastEventPollAt` (o `connectedAt` en el primer poll — nunca barre el historial completo de la cuenta) y se los pasa uno por uno a `processStripeWebhookEvent`. Un tenant que falla (401/403 → `markNeedsAttention`, o cualquier otro error) no frena a los demás.
