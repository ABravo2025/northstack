...
// (legacy signups predating spec-tenant-signup.md's required country field), defaults to Dodo
// Payments.
...
// Subscription. Dodo Payments-only (see getInvoiceUrl's comment in dodopayments.ts); Mercado Pago
// invoices have no equivalent document yet.
export async function getInvoiceDocumentUrl(tenantId: string, invoiceId: string): Promise<GetInvoiceDocumentUrlResult> {
  ...
  const url = await getInvoiceUrl(invoice.externalInvoiceId);
  if (!url) {
    return { success: false, error: 'No document available for this invoice yet.' };
  }
