...
### `src/lib/dodopayments.ts` (Billing Integration — reemplazó a `paddle.ts`, 2026-09-13)
A diferencia de `mercadopago.ts`/`stripe.ts` (hand-rolled a propósito), este usa el SDK oficial
`dodopayments` (npm) — no había nada "verificado contra sandbox real" todavía para Dodo al momento
de escribirlo, así que se apoya en los tipos oficiales del SDK (confirmados contra
`dodopayments@2.50.0`) en vez de reimplementar los request/response shapes a mano. Ver el
comentario al principio del archivo para el razonamiento completo.
- **createCheckoutSession(input)** — `client.checkoutSessions.create()`, `product_cart: [{product_id, quantity: 1}]` (Dodo exige un Product de catálogo pre-creado, a diferencia del precio inline que aceptaba Paddle — ver `scripts/setup-dodo-products.ts`). `input.subscriptionId` va en `metadata.subscriptionId`, la clave de join del webhook (mismo rol que `custom_data` de Paddle / `external_reference` de MP). `input.trialDays?` → `subscription_data.trial_period_days`. Devuelve `checkout_url` — Dodo redirige a una página hosteada, no abre un overlay embebido como Paddle.js.
- **getCustomerPortalUrl(externalSubscriptionId, returnUrl)** — reemplaza el rol de `getUpdatePaymentMethodTransaction` de Paddle: lee la subscription (`client.subscriptions.retrieve`) para obtener `customer.customer_id`, y abre una sesión de Customer Portal (`client.customers.customerPortal.create`) donde el cliente reemplaza su tarjeta sin crear una segunda suscripción.
- **changeSubscriptionPlan(externalSubscriptionId, { productId })** — `client.subscriptions.changePlan()`, `proration_billing_mode: 'do_not_bill'` (mismo "sin prorrateo" que Paddle).
- **cancelSubscription(externalSubscriptionId)** / **removeScheduledCancellation(externalSubscriptionId)** — `client.subscriptions.update()` con `cancel_at_next_billing_date: true`/`false` (equivalente a `effective_from: next_billing_period` / `scheduled_change: null` de Paddle).
- **getInvoiceUrl(externalPaymentId)** — `client.payments.retrieve(id).invoice_url`. A diferencia de Paddle, Dodo devuelve la URL directo en el Payment, sin un endpoint/param de `disposition` separado.
- **getNextBillingDate(externalSubscriptionId)** — `client.subscriptions.retrieve(id).next_billing_date`, usado por el webhook para un `currentPeriodEnd` preciso (Paddle no tenía un llamado barato equivalente, usaba una aproximación de +30 días).
- **getSubscriptionProductId(externalSubscriptionId)** (2026-09-16, QA-91) — `client.subscriptions.retrieve(id).product_id`. Fallback de `routes/webhooks.ts`'s `resolveConfirmedPlan()` cuando `metadata.plan` no viene en el evento — la propagación de metadata custom de Dodo hacia eventos posteriores resultó no ser confiable en la práctica (encontrado en vivo), a diferencia de `product_id`, dato estructural del recurso Subscription. Ver también **resolvePlanFromDodoProductId(productId)** en `subscriptionService.ts` (reverse-lookup contra `PlanPrice.dodoProductId`, la dirección opuesta a como `checkoutService.ts` ya lo usaba).
- **createRecurringProduct(input)** — `client.products.create()`, solo usado por `scripts/setup-dodo-products.ts` para provisionar un Product de catálogo por cada fila de `PlanPrice` (`market: 'international'`).
- **unwrapDodoWebhookEvent(rawBody, headers)** — `client.webhooks.unwrap()`, que internamente usa el paquete `standardwebhooks` (ya viene como dependencia transitiva de `dodopayments`, no hace falta instalarlo aparte) para verificar la firma "Standard Webhooks" (headers `webhook-id`/`webhook-signature`/`webhook-timestamp`) — reemplaza a `verifyPaddleSignature`'s hand-rolled HMAC. Tira si `DODO_WEBHOOK_KEY` no está configurada o la firma no valida.
...
### `src/routes/externalApi.ts` (Private API + Webhooks, Units 2-3 — 2026-09-07)
Router de rutas `/api/external/v1/*` — no reimplementa lógica, envuelve los servicios ya
...
con `prismaExternal` en vez del `prisma` compartido. Piezas reusables:
...
- **respond(req, res, apiKey, status, body)** — escribe el `ApiRequestLog` (vía `bestEffort`,
  **awaited antes de responder**, nunca en un listener `res.on('finish')` — esa función podría
  ...
  fire-and-forget, ver `bestEffort.ts`) y recién ahí manda la respuesta. `status: 204` hace
  `res.end()`, nunca `res.json({})` (un 204 no puede llevar body, RFC 7231).
- **parseBody(req, res, apiKey, schema)** (Unit 3) — primer uso de `zod` en el proyecto. Parsea
  `req.body ?? {}` (un DELETE sin body/Content-Type deja `req.body` en `undefined`, no `{}` — el
  fallback evita que un schema de puros opcionales falle igual) contra el `schema` dado; en error,
  400 `{code: 'validation_error', details: [{path, message}, ...]}` y devuelve `null`.
- **isoDateTime** / **dateString** (Unit 3) — dos validadores zod para campos de fecha: el primero
  exige ISO 8601 completo con hora (Task.dueDate, Opportunity.estimatedCloseDate — son un momento
  puntual); el segundo es más laxo, solo exige que `new Date(...)` no dé `NaN` (Time Off
  startDate/endDate son fechas de calendario, "2026-09-10", sin componente de hora).
- **requireScopeLogged** / **badRequest** / **notFound** / **verifyCrossModuleEntity** (Unit 3) —
  helpers de escritura: el primero es la versión de `requireScope` (externalApiAuth.ts) que además
  loggea antes del 403; el segundo/tercero arman `{error, code}` consistentes; el cuarto valida que
  un `entityType`/`entityId` de Task/Note pertenezca al tenant (mismo chequeo anti-IDOR que
  `routes/tasks.ts`/`routes/notes.ts` ya hacen antes de crear).
- Cada recurso de escritura reusa la validación de referencias/reglas de negocio que ya existía del
  lado interno en vez de reimplementarla: **validateOpportunityRefs** (`routes/opportunities.ts`,
  exportada) y **validateContactRefs** (`routes/contacts.ts`, exportada). Company/Employee no tenían
  una función de validación separada para exportar — sus checks (`accountOwnerId`, ciclo de
  jerarquía, `departmentId`/`jobTitleId`, ciclo de manager, enum de `contractType`/`personType` vía
  **VALID_CONTRACT_TYPES**/**VALID_PERSON_TYPES** exportadas de `routes/employees.ts`) están inline
  en los handlers, calcados de `routes/companies.ts`/`routes/employees.ts`.
- **Sin endpoint dedicado de cambio de stage para Opportunity** — el task breakdown original pedía
  `PATCH .../opportunities/:id/stage` separado, asumiendo que la API interna ya lo separaba así; no
  es cierto (`routes/opportunities.ts` mueve stage vía el PATCH genérico, igual que el drag-and-drop
  de Kanban — ver `contexto-proyecto.md`, sección 2026-07-16). El PATCH genérico externo acepta
  `stageId` igual que el interno, con el mismo chequeo de win/loss-reason vía `validateOpportunityRefs`.
- **hr.timeoff** solo tiene `POST` (crear una solicitud) — sin decidir/cancelar vía API: esas
  acciones dependen de "es el manager asignado", una relación que una ApiKey no tiene (spec
  decisión #4). **hr.payroll:write no existe** — Alejandro, 2026-09-07: el scope de mayor riesgo del
  catálogo queda fuera de v1 en vez de requerir un paso extra de confirmación; `createApiKey` lo
  rechaza como scope desconocido.
- Cada `create*`/`update*`/`delete*` llamado acá atribuye la acción a **`apiKey.createdByUserId`**
  (quien creó la key) — una ApiKey no es un `User`/sesión, así que no tiene un actor propio para los
  parámetros `createdById`/`changedByUserId` que cada service ya exige.
...
CRUD estándar: **createCompany(input, changedByUserId, client?)**, **listCompanies(tenantId, client?)**, **findCompanyById(id, client?)**, **updateCompany(id, input, changedByUserId, client?)**, **deleteCompany(id, changedByUserId, options?, client?)**. Los 3 de escritura registran una entrada en Activity Log (`docs/general/spec-activity-log.md`, 2026-08-30) vía `companyActivityFieldConfig`. `client?` (Private API + Webhooks Units 2-3, 2026-09-07) acepta `prismaExternal` en vez del `prisma` compartido — ver `routes/externalApi.ts`.

### `src/modules/crm/contactService.ts`
CRUD estándar: **createContact(input, changedByUserId?, client?)**, **listContacts(tenantId, includeInactive?, client?)**, **findContactById(id, client?)**, **updateContact(id, input, changedByUserId, client?)**, **deactivateContact(id, changedByUserId, client?)** (soft-delete, Sales v2 — reemplazó el `deleteContact` legado). Los 3 de escritura registran Activity Log vía `contactActivityFieldConfig` — `createContact` no loguea si `changedByUserId` viene vacío (caso de `publicFormService.ts`, sin usuario autenticado). `client?` (Private API + Webhooks Units 2-3) acepta `prismaExternal` — ver `routes/externalApi.ts`. **validateContactRefs(tenantId, body)** (`routes/contacts.ts`, exportada en Unit 3 para reuso) — valida `companyId`/`leadStatus`/`leadSourceId` referenciados en el body.

### `src/modules/crm/opportunityService.ts`
- CRUD estándar: **createOpportunity(input, changedByUserId?, client?)**, **listOpportunities(tenantId, includeInactive?, client?)**, **findOpportunityById(id, client?)**, **updateOpportunity(id, tenantId, input, client?)** (`input.changedByUserId` opcional), **deleteOpportunity(id, changedByUserId, client?)**. Los 3 de escritura registran Activity Log vía `opportunityActivityFieldConfig` — mismo criterio de `changedByUserId` opcional en `create` que `contactService.ts`. `client?` (Private API + Webhooks Units 2-3) acepta `prismaExternal` — ver `routes/externalApi.ts`. **validateOpportunityRefs(...)** (`routes/opportunities.ts`, exportada en Unit 3) — valida Pipeline activo, Company/gate de tipo de pipeline, `ownerId`, y la regla win/loss-reason según el stage efectivo; usada tanto por el PATCH interno como por el externo (no hay endpoint de cambio de stage separado en ninguno de los dos — cambiar `stageId` es un campo más del PATCH genérico).
...
- **createEmployee(input, changedByUserId?, client?)**, **listEmployees(tenantId, visibleIds?, client?)** (suma `contractStatus` por fila — Unidad 11; `visibleIds` es Fase C, Custom Roles — `Set<string> | null | undefined`, filtra el `where` a esos ids cuando se pasa, sin filtrar cuando es `null`/`undefined`; el router externo, Unit 2, siempre pasa `null` — el scope de la ApiKey es la única autorización, el scope de Employee por-persona no aplica a ese tráfico), **findEmployeeById(id, client?)**, **findEmployeeByUserId(userId)**, **updateEmployee(id, input, changedByUserId, client?)**, **deleteEmployee(id, changedByUserId, client?)**. `client?` (Private API + Webhooks Units 2-3) acepta `prismaExternal`. **VALID_CONTRACT_TYPES**/**VALID_PERSON_TYPES** (`routes/employees.ts`, exportadas en Unit 3) — los 2 enums chicos que tanto la ruta interna como la externa validan contra. Los 3 de escritura registran Activity Log (`docs/general/spec-activity-log.md`, 2026-08-30) vía `employeeActivityFieldConfig` — `changedByUserId` opcional solo en `createEmployee`: solo la ruta directa (`POST /api/hr/employees`) lo pasa hoy, así que solo esa genera entrada; CSV import, onboarding seed data y `publicFormService.ts` la llaman sin ese argumento a propósito (scope cut de Unidad 2, ver el spec) y no generan ninguna.
...
- **createTimeOffRequest(input, changedByUserId, client?)** — valida fechas + asignación de política, auto-aprueba si la política no requiere aprobación. Desde 2026-08-22, dispara `syncTimeOffCalendarEvent` (best-effort) si el resultado ya nace `approved`. `client?` (Private API + Webhooks Unit 3, 2026-09-07) acepta `prismaExternal` — `POST /api/external/v1/hr/timeoff` es el único write de Time Off expuesto (sin decidir/cancelar vía API: eso depende de "es el manager asignado", una relación que una ApiKey no tiene).
...
### `src/modules/integrations/googleCalendarSyncService.ts` (2026-08-22, rediseño de Time Off 2026-08-23, Google Meet 2026-09-16)
- **syncTaskCalendarEvent(previous, current)** — sync unidireccional (Northstack → Google) del Task al calendario de su `assigneeId` únicamente (personal, 1 registro → 1 evento). Best-effort, nunca tira. Llamada desde `taskService.ts` tras create/update/delete. Usa `taskEventBody` (no exportada) para decidir evento de todo el día (`dueDate` en medianoche UTC exacta, sin hora elegida) vs. evento con hora (`dueDate` con hora real, bloque de 1h fijo en el calendario) — 2026-08-23, pedido de Alejandro tras ver que Google Calendar da mejor perspectiva con hora que solo con el día. Desde 2026-09-16 también resuelve `resolveTaskAttendeeEmail` (no exportada — Contact y Employee usan su propio email; Company/Opportunity usan el email del Contact `isPrimary` de esa Company, o ninguno si no hay uno marcado) y, si `Task.hasVideoCall` y todavía no tiene `googleMeetUrl`, pide a Google un Meet nuevo (`conferenceData.createRequest`, `conferenceDataVersion: 1`) — el link resultante (`data.hangoutLink`) se persiste una sola vez, nunca se regenera ni se borra si `hasVideoCall` se destilda después. El email resuelto (si lo hay) entra como `attendees` + `sendUpdates: 'all'` para que esa persona reciba la invitación de Calendar automáticamente, tenga o no Meet.
...
- **listGoogleEventsForCalendarView(userId, timeMinISO, timeMaxISO)** (2026-08-27, extendido 2026-09-16) — overlay de solo lectura para el calendario del Overview: eventos del Google Calendar del usuario que **no** son Tasks ya sincronizadas (esas ya se muestran aparte, vía `calendarTasks`; listarlas de nuevo acá las duplicaría). Nunca crea nada en Northstack ni empuja nada de vuelta a Google. `GoogleCalendarViewEvent` trae `title`/`start`/`end`/`allDay` + (desde 2026-09-16) `description`/`location`/`htmlLink`/`hangoutLink` — todo leído directo del recurso de Google, nada nuevo que pedirle a la API — para el modal chico de "ver evento" del Overview (`GoogleEventViewModal.tsx`).
...
CRUD estándar, cross-entidad vía `entityType`/`entityId`: **createNote(input, client?)**, **findNoteById(id, client?)**, **listNotesForEntity(tenantId, entityType, entityId)**, **updateNote(id, input, changedByUserId, client?)**, **deleteNote(id, changedByUserId, client?)**. `client?` (Private API + Webhooks Unit 3) acepta `prismaExternal`.
...
- CRUD cross-entidad: **createTask(input, client?)**, **findTaskById(id, client?)**, **listTasksForEntity(tenantId, entityType, entityId)**, **updateTask(id, input, changedByUserId, client?)**, **deleteTask(id, changedByUserId, client?)** — las tres primeras (create/update/delete) disparan `syncTaskCalendarEvent` (best-effort, 2026-08-22) tras la escritura. `client?` (Private API + Webhooks Unit 3) acepta `prismaExternal`. `input.hasVideoCall?`/`UpdateTaskInput.hasVideoCall?` (2026-09-16) — pide un Google Meet en el próximo sync; ver `googleCalendarSyncService.ts`.
...
- **listTasksForUser(tenantId, userId, opts?)** (2026-09-18, My Tasks hub) — toda Task donde el usuario es assignee **o** creador (`OR` en el where, no solo assignee como `listMyTasks`), con filtros opcionales `includeCompleted`/`folderId`/`search`. Adjunta `entitySummary` (igual que `listMyTasks`) y `relationship` (`'assignee'|'creator'|'both'|null`) por tarea. Backea `GET /api/tasks/mine?scope=hub` — sin ese query param, la ruta sigue llamando a `listMyTasks` sin cambios, así el widget "My tasks" de Overview no se ve afectado.
- `Task.folderId`/`Task.folder` (schema, 2026-09-18) — FK opcional a `TaskFolder` (`onDelete: SetNull`), field nuevo en `CreateTaskInput`/`UpdateTaskInput`, incluido en `taskInclude`.

### `src/modules/tasks/taskFolderService.ts` (2026-09-18, My Tasks hub)
- **listTaskFoldersForTenant(tenantId)** — todas las `TaskFolder` del tenant + `pendingTaskCount` en vivo (Task no completada por folder, vía `groupBy`, mismo patrón que `platformTenantService.ts`) — evita N+1 desde el rail de carpetas del frontend.
- **createTaskFolder(input)** / **findTaskFolderById(id)** / **deleteTaskFolder(id)** — `deleteTaskFolder` pone `folderId: null` en toda Task de esa carpeta antes de borrarla (transacción), no confía solo en el `onDelete: SetNull` del FK.
...
- **resolveProvider(tenant)** — `tenant.country === 'Argentina'` → `mercadopago`; cualquier otro valor, incluido `null` (tenants legacy sin país), → `dodopayments` (Paddle hasta el 2026-09-13, ver nota en `spec-billing-integration.md`).
- **resolvePlanFromDodoProductId(productId)** (2026-09-16, QA-91) — `'starter'|'growth'|null` a partir de un `dodoProductId` de Dodo, reverse-lookup contra `PlanPrice`. Fallback de `routes/webhooks.ts`'s `resolveConfirmedPlan()` cuando `metadata.plan` no viene en el evento — ver `getSubscriptionProductId` en `dodopayments.ts`.
- **syncSubscriptionAndTenant(input)** — único punto de escritura de `Subscription` + su espejo en `Tenant.status`/`plan`/`trialEndsAt`/`gracePeriodEndsAt`/`lockedPriceCents`, en una sola transacción. Pensado para el cron y los dos webhook handlers (Dodo Payments/Mercado Pago) — `updateTenantPlan` de arriba es la única excepción deliberada (escribe ambas filas inline, no es una transición de status). Solo escribe en `Tenant` los campos del subset con espejo real, y solo si vinieron seteados en el `input` — un webhook que solo toca `paymentMethodBrand`/`Last4` nunca toca `Tenant`. **2026-08-31**: sumó `changedByUserId?` opcional al input y registra Activity Log al final — si viene `changedByUserId` (los 3 self-serve de abajo, síncronos) lo usa directo; si no (todo call site de webhook/cron, que nunca tiene actor propio), cae al puntero `Subscription.lastActionByUserId`/`lastActionAt` (helper privado `resolveRecentActorId`, ventana de 60 min — `SUBSCRIPTION_ACTOR_TRUST_WINDOW_MS`) solo si sigue fresco; si ninguno aplica, no loguea nada. Único lugar donde vive esta lógica — `webhooks.ts`/`planTransitionService.ts` no cambiaron. Ver también **recordSubscriptionActionAttempt(tenantId, userId)** (mismo archivo) — escribe solo `lastActionByUserId`/`lastActionAt`, sin tocar estado de facturación; usada por `startCheckout`.
- **getBillingSummary(tenantId)** (Etapa E) — backea `GET /api/subscriptions/me`, lectura del `Subscription` + sus `invoices` para `BillingPage.tsx`. No estaba en el breakdown original (todas las unidades 1-15 son escrituras) — sin esto la página no tiene de dónde leer. Cualquier miembro autenticado del tenant, mismo criterio que `GET /api/tenants/current` (los endpoints que escriben siguen owner-only vía `canManageBilling`). **2026-09-15 (QA-88)**: ahora también arma y devuelve el contador de seats en vivo (`tenantPlan`, `activeSeats`, `includedSeats`, `extraSeats`, `extraSeatsCostCents`, via `seatService.ts`) — `tenantPlan` (no `subscription.plan`, que puede seguir siendo el placeholder `'starter'` del signup) es la fuente real de "qué plan tiene de verdad" para el frontend, `null` significa Free Trial sin confirmar todavía. El branch de self-heal (tenant pre-Billing-Integration sin fila de `Subscription`) nunca dispara para un tenant nuevo — `registerTenantWithOwner` ya crea esa fila siempre.
- **getInvoiceDocumentUrl(tenantId, invoiceId)** (2026-08-19, sin `disposition?` desde 2026-09-13 — Dodo no tiene ese concepto) — backea `GET /api/subscriptions/me/invoices/:invoiceId/document`. Verifica que la Invoice pertenezca de verdad al tenant antes de pedirle la URL a `getInvoiceUrl` (`dodopayments.ts`) — Dodo Payments-only, Mercado Pago no tiene documento equivalente todavía.

### `src/modules/tenant/seatService.ts` (Seats pricing, 2026-09-14, extendido 2026-09-15 QA-88)
- **INCLUDED_SEATS** — `{ starter: 5, growth: 10 }`. **EXTRA_SEAT_PRICE_CENTS** — `400` ($4/mo, plano, sin distinción de rol). **FREE_TRIAL_SEAT_CAP** — `5`, tope duro para un tenant sin plan elegido (`Tenant.plan === null`), el único caso donde no hay forma de cobrar overage (sin provider/tarjeta todavía).
- **countActiveSeats(tenantId)** — cuenta `User` con `status: 'active'` del tenant; un `Employee` sin `User` vinculado (solo tracking de Payroll) nunca cuenta.
- **extraSeatsFor(plan, activeSeats)** — `Math.max(0, activeSeats - INCLUDED_SEATS[plan])`, solo `'starter'|'growth'`.
- **syncSeatBilling(tenantId)** — sincroniza la cantidad de la addon de seat extra en el proveedor real (Dodo: `updateSubscriptionSeats`, prorateado de inmediato; Mercado Pago: recalcula el `transaction_amount` completo, solo afecta el próximo cobro). No-op sin plan starter/growth, sin `provider`/`externalSubscriptionId`, o mientras `status === 'trialing'` (no se cobra overage durante el trial — el webhook de primer cobro real hace el true-up). Call sites: `invitationService.ts` (invite aceptado), `tenantUserService.ts` (cambio de status), `terminationService.ts` (baja), `webhooks.ts` (primer cobro real), `contractConfirmationService.ts` (confirmación de contrato de Payroll — **2026-09-15**, gap encontrado y cerrado junto con el cap de abajo, nunca se llamaba acá antes).
- **seatCapError(tenant)** (2026-09-15, QA-88) — `null` si `tenant.plan` no es `null` (ya tiene plan real, el overage se cobra en vez de bloquearse) o si `activeSeats < FREE_TRIAL_SEAT_CAP`; si no, un mensaje de error listo para devolver como `{ success: false, error }` (nunca throwea, mismo contrato que el resto de esta familia de módulos). Call sites: `invitationService.ts` (`createInvitation` y `acceptInvitation`, chequeado dos veces — al crear la invitación para avisar de inmediato, y al aceptarla por si los seats se llenaron mientras tanto), `tenantUserService.ts` (`updateTenantUser`, al reactivar un usuario suspendido), `contractConfirmationService.ts` (confirmar contrato también crea un seat activo nuevo).

### `src/modules/tenant/checkoutService.ts` (Billing Integration, `docs/general/spec-billing-integration.md`)
- **startCheckout(tenant, user, requestedPlan?)** — backea `POST /api/subscriptions/me/checkout`. Desde 2026-08-19 (corrección de Alejandro) cubre dos intents distintos según si `Subscription.provider` ya está seteado: **suscribirse por primera vez** (sin provider — crea una Checkout Session/Preapproval nueva, no toca `Subscription` hasta que el webhook confirme el pago) vs. **actualizar el método de pago** de una suscripción YA activa (con provider — Dodo Payments usa `getCustomerPortalUrl` sobre la MISMA suscripción; Mercado Pago no tiene ese mecanismo así que cancela la preapproval vieja y crea una nueva, mismo resultado neto de "una sola tarjeta activa"). Nunca crea una segunda suscripción compitiendo para un tenant ya activo. Rechaza si el `PlanPrice` del mercado del tenant no existe, es el placeholder AR (0 cents), o (para Dodo) no tiene `dodoProductId` provisionado todavía. **2026-09-15 (QA-88)**: nuevo parámetro `requestedPlan` — para una suscripción nueva ya NO lee `subscription.plan` (el placeholder `'starter'` del signup) sino que exige y usa este parámetro para toda la lógica de precios/producto, así `Tenant.plan`/`Subscription.plan` pueden quedarse sin tocar hasta que el pago se confirme de verdad (antes, `BillingPage`/`PlansModal` llamaban `updateTenantPlan` ANTES del checkout, mostrando un plan "elegido" que nunca se llegó a pagar). Para Dodo, `requestedPlan` viaja en `metadata.plan` de la Checkout Session (`dodopayments.ts`), leído de vuelta por el webhook (`routes/webhooks.ts`) recién cuando el pago se confirma. Mercado Pago no tiene un canal de metadata libre en su preapproval (solo `external_reference`, ya ocupado por el `subscriptionId`) — para ese proveedor el plan se sigue escribiendo de inmediato vía `updateTenantPlan`, asimetría aceptada porque el mercado AR todavía no está en producción real (precios `PlanPrice` en 0). **2026-08-20**: la rama de suscripción nueva pasa `trialDays: SIGNUP_TRIAL_DAYS` (de `tenantService.ts`) a `createPreapproval`/`createCheckoutSession` — tarjeta ahora, primer cobro real recién a los 15 días. La rama de "actualizar método de pago" (incluido el fallback de cancelar+recrear de MP) nunca pasa `trialDays` — no le corresponde un trial nuevo a alguien que ya está pagando. **2026-08-20 (misma tarde)**: ese `trialDays` real solo aplica cuando `process.env.BILLING_ENV === 'production'` (`isRealProductionBilling`, la variable se llamaba `PADDLE_ENV` hasta el 2026-09-13) — fuera de eso (staging, local dev, cualquier lugar corriendo contra sandbox) una suscripción nueva cobra de inmediato en vez de dar los 15 días, para poder confirmar el flujo completo tarjeta→webhook→suscripción activa contra sandbox sin esperar 15 días reales. Una sola variable para ambos proveedores en vez de una por proveedor, porque en la práctica los dos pasan a modo real juntos. **2026-08-21 (catch de Alejandro)**: `trialDays` ya no es siempre `SIGNUP_TRIAL_DAYS` fijo — se calcula como los días que de verdad quedan hasta el `Tenant.trialEndsAt` ORIGINAL (fijado una sola vez al signup), tope `SIGNUP_TRIAL_DAYS`, y si ya venció (`daysRemaining <= 0`) cobra de inmediato en vez de dar más tiempo gratis. Sin esto, como nada acá escribe `Subscription.provider` hasta que un webhook confirma el pago, alguien podía arrancar-y-abandonar el checkout indefinidamente y, cuando finalmente completara uno, siempre conseguía 15 días frescos desde ESE momento — empujando el primer cobro real para siempre, el trial "interminable" que señaló. `startCheckout` ahora recibe `tenant.trialEndsAt` (antes solo `id`/`country`) — `routes/subscriptions.ts` lo agregó al `select`. **2026-08-31**: el segundo parámetro pasó de `{ email }` a `{ id, email }` — llama `recordSubscriptionActionAttempt(tenant.id, user.id)` justo después de confirmar que existe `subscription`, para que el webhook que confirme el pago después pueda atribuirle el cambio a este usuario (ver `syncSubscriptionAndTenant`). Es la única escritura de metadata (no de estado de facturación) que hace en el camino "subscribe" — sigue sin tocar `provider`/`externalSubscriptionId`/etc ahí.

### `src/modules/tenant/subscriptionSelfServeService.ts` (Billing Integration, Etapa D)
- **changePlan(tenantId, plan, userId)** — solo si la `Subscription` ya tiene `provider`/`externalSubscriptionId` (si no, usar `PATCH /api/tenants/me/plan` en su lugar). Llama al proveedor correspondiente (Dodo: `changeSubscriptionPlan` con el `dodoProductId` del nuevo plan) y, confirmada esa respuesta, actualiza `plan`/`lockedPriceCents` YA MISMO vía `syncSubscriptionAndTenant` (decisión confirmada con Alejandro: la respuesta del proveedor ya es la confirmación, no hace falta esperar el próximo webhook de cobro — no se agregó ninguna columna nueva de "plan pendiente"). **2026-08-31**: sumó `userId` (ya estaba en scope en la ruta, solo faltaba pasarlo) — se lo pasa a `syncSubscriptionAndTenant` como `changedByUserId`, así queda logueado con el actor real, no con el fallback de correlación.
- **requestCancellation(tenantId, reason?, userId)** — Dodo Payments: llama `cancelSubscription` en el momento (cancelación nativa programada, `cancel_at_next_billing_date: true`). Mercado Pago: no llama a nada — el barrido del cron (`planTransitionService.ts`) lo hace cuando vence `cancellationEffectiveAt`. Ninguno de los dos toca `Tenant.status` acá. **2026-08-31**: mismo agregado de `userId`/`changedByUserId` que `changePlan`.
- **resumeSubscription(tenantId, userId)** — Dodo Payments: llama `removeScheduledCancellation` (gap real encontrado: la spec decía "no hace falta llamar a ningún proveedor", cierto solo para MP). Mercado Pago: no llama a nada. **2026-08-31**: mismo agregado de `userId`/`changedByUserId` que `changePlan`.
...
| `tasks.ts` | listTasks, listMyTasks (2026-08-22: excluye completadas), listTasksHub (My Tasks hub, `?scope=hub`, 2026-09-18), listTasksForCalendar (ídem), createTask, updateTask, deleteTask |
| `taskFolders.ts` | listTaskFolders, createTaskFolder, deleteTaskFolder (2026-09-18) |
...
| `billing.ts` (Billing Integration, Etapa E) | getSubscription (2026-09-15: `Subscription` ahora trae `tenantPlan`/`activeSeats`/`includedSeats`/`extraSeats`/`extraSeatsCostCents`, ver `types.ts`), startCheckout(token, plan?) (2026-09-15: `plan` obligatorio para una suscripción nueva — ver `checkoutService.ts`'s `requestedPlan`; `updateTenantPlan` de `auth.ts` YA NO se llama desde ningún lado del frontend antes del checkout, sigue existiendo por si se necesita después), changeSubscriptionPlan (post-billing, distinto de `updateTenantPlan` de arriba que es la elección pre-billing durante trial), cancelSubscription, resumeSubscription, getInvoiceDocumentUrl(token, invoiceId) (2026-08-19, Dodo Payments-only, se pide fresca en cada click — `BillingPage.tsx` la usa para el link "View invoice" por fila de Invoice; sin `disposition?` desde 2026-09-13, Dodo no tiene ese concepto) |
...
- **AddPaymentMethodModal** (2026-08-19, Billing Integration) — dispara `POST /api/subscriptions/me/checkout`; ambos proveedores abren en pestaña nueva vía `window.open(initPoint, ...)` — desde 2026-09-13 los dos devuelven una URL hosteada directo (`initPoint`: Mercado Pago's `init_point`, Dodo's `checkout_url`/Customer Portal link), sin ninguna página intermedia propia. Nunca arma un form de tarjeta propio. Prop `mode: 'subscribe' | 'update'` cambia el copy (elegir plan por primera vez vs. reemplazar la tarjeta de una suscripción ya activa — dos intents distintos, corrección de Alejandro). **2026-08-20 (corrección, Paddle-era)**: el componente dejó de cargar `paddle.js`/llamar `Paddle.Checkout.open()` en la pestaña actual — Alejandro pidió que el checkout se sienta como su propia ventana, no un overlay apilado sobre la actual (por entonces vía una `PaddleCheckoutPage.tsx` dedicada en `/billing/checkout?transactionId=...`, borrada 2026-09-13 junto con Paddle — Dodo no necesita una página intermedia, ya entrega una URL hosteada como Mercado Pago); el componente ya no tiene prop `onCompleted` (no hay señal de vuelta a la pestaña original — `BillingPage.tsx` refetchea al recuperar foco en su lugar, ver abajo). **2026-08-21 (corrección)**: la regla "si no hay modal, pestaña nueva" valía para los dos proveedores — Mercado Pago todavía hacía `window.location.href = initPoint` (navegaba la pestaña actual fuera de Northstack por completo); ahora `window.open(initPoint, '_blank', 'noopener,noreferrer')` para cualquier proveedor que devuelva `initPoint`. **2026-08-21 (misma tarde)**: nueva prop `trialDaysRemaining?: number` (solo relevante en `mode="subscribe"`) — el copy y el título ("Start your free trial" vs. "Subscribe") ahora reflejan si de verdad queda trial o no, en vez de prometer siempre "15 días" sin importar cuánto tiempo ya pasó; espejo exacto de lo que `checkoutService.ts` va a cobrar de verdad (ver su entry abajo). Montado tanto en `AppLayout.tsx` (banner de `past_due`/`suspended`, siempre `mode="subscribe"`, `trialDaysRemaining` calculado sobre `tenant.trialEndsAt` — normalmente 0 ahí, porque para llegar a ese banner el trial ya venció) como en `BillingPage.tsx` (modo según si ya hay `provider`, `trialDaysRemaining` sobre `subscription.trialEndsAt`).
...
- **Icons.tsx** — toda la iconografía de la app, un componente por ícono (`SearchIcon`, `PlusIcon`, `PencilIcon`, `TrashIcon`, `MailIcon`, `EyeIcon`/`EyeOffIcon`, `CheckIcon`, `XIcon`, `GripIcon`, `GridIcon`, `KanbanIcon`, `ListIcon`, `LockIcon`, `TeamIcon`, `FilterIcon`, `DotsVerticalIcon`, `CopyIcon`, `HomeIcon`, `DashboardIcon`, `CalendarIcon`, `TrendingIcon`, `PeopleIcon`, `BriefcaseIcon`, `GearIcon`, `UserCircleIcon`, `ChevronDownIcon`/`ChevronLeftIcon`/`ChevronRightIcon`, `MenuIcon`, `DownloadIcon`, `UploadIcon`, `BellIcon`, `BuildingIcon`, `TargetIcon`, `SparklesIcon`, `CreditCardIcon`, `ClockIcon`, `VideoIcon` (2026-09-16, "Join call" en Tasks), `RocketIcon`, `PlugIcon`, `FormIcon`, `DeviceIcon`, `InfoIcon`, `AlertTriangleIcon`, `AlertCircleIcon`) — **revisar esta lista antes de agregar un ícono nuevo**, es fácil duplicar uno que ya existe con otro nombre. (Esta lista estaba desactualizada respecto al archivo real hasta 2026-09-16 — quedaban 11 íconos sin documentar; se regeneró completa en esa fecha.)
...
- **TaskForm** — form de compose/edit de un Task, siempre expandido dentro del tab. Checkbox "Add Google Meet video call" (2026-09-16, `TaskFormPayload.hasVideoCall`) — si se tilda sin fecha/hora cargada, autocompleta hoy + 10:00 (`DEFAULT_CALL_TIME`); si se borra la fecha o la hora, se destilda solo. Muestra el link "Join Google Meet" (`task.googleMeetUrl`) una vez que Google lo generó. **2026-09-18 (My Tasks hub)**: dos props nuevas, ambas opcionales y sin efecto en los call sites existentes si se omiten — `folders?`/`defaultFolderId?` (renderiza un select "Folder" e incluye `folderId` en el `TaskFormPayload` solo si `folders` viene seteado) y `googleCalendarConnected?` (default `true`; en `false` reemplaza el checkbox de Meet por un aviso "Connect Google Calendar in Settings..."). **Este gate se extendió el mismo día a los 4 lugares que ofrecen el checkbox** (no solo el hub) — ver `useGoogleCalendarConnected` abajo.
- **hooks/useGoogleCalendarConnected(token)** (2026-09-18) — `boolean`, wrapea `api.getGoogleCalendarStatus` (`connected && !needsReconnect`). Usado por `MyTasksPage`, `EntityTasksList`, `MyTasksWidget` y `NewTaskFromCalendarPopover` para pasarle el prop `googleCalendarConnected` real a `TaskForm`/`TaskFormPopover` en cada uno — antes de esto el checkbox de Meet se mostraba siempre, aunque tildarlo sin Calendar conectado nunca iba a producir un link.
- **TaskFormPopover** — wrapper de `TaskForm` en un `Popover`, para los 2 lugares que sí necesitan popover-al-click (el widget de Overview y las entradas del calendario) — nunca reimplementar el form ahí adentro, envolver el mismo `TaskForm`. Prop `googleCalendarConnected?` (2026-09-18) pasa directo a `TaskForm`.
- **GoogleEventViewModal** (2026-09-16) — modal chico de solo lectura para un evento de Google Calendar "crudo" (no Task) del overlay del Overview (`calendar-entry-google`, antes no hacía nada al clickear). Muestra fecha/hora formateada, ubicación, descripción, y los links "Join Google Meet" (si `hangoutLink`)/"Open in Google Calendar" (`htmlLink`) — nunca editable acá, la edición real vive en `calendar.google.com`.

#### My Tasks hub (`/tasks`, 2026-09-18) — módulo nuevo, ver `pages/MyTasksPage.tsx`
Página nueva bajo Overview en el nav (`Sidebar.tsx`) — toda Task donde el usuario es assignee o creador, List/Board, carpetas, búsqueda. Aprobada primero como mockup (Artifact canvas) antes de esta implementación real. Deliberadamente separada en varios archivos chicos (pedido explícito, no un solo archivo grande):
- **hooks/useTaskFolders.ts** — list/create/delete de `TaskFolder`, usado por el rail y por los selects "Folder" de los modales.
- **hooks/useMyTasksHub.ts** — fetch de tasks (`api.listTasksHub`) + filtros (`showCompleted`/`folderFilter`/búsqueda debounced 300ms) + `toggleComplete`/`deleteTask`.
- **hooks/useEntityPicker.ts** — picker compartido "tipo de entidad → record" (Company/Contact/Employee/Opportunity), extraído de `NewTaskFromCalendarPopover.tsx` para no duplicarlo en `NewTaskModal.tsx` — **de paso sumó soporte para Opportunity**, que `NewTaskFromCalendarPopover` no tenía.
- **components/tasks/TaskFoldersSidebar** — rail colapsable de carpetas ("All tasks" + cada `TaskFolder`, con conteo pendiente en vivo), + alta inline de carpeta nueva.
- **components/tasks/TaskHubList** / **TaskHubRow** — vista lista sobre la tabla compartida `table full-table` (mismo estilo que People/Companies: encabezados chicos, `.name-cell`, `.avatar`, fila fantasma "Add"), sin agrupar por día/semana (a pedido explícito), ordenada por fecha, columnas Relationship/Created/Due/Completed/Assignee. Título de la fila siempre arranca con `entitySummary` en negrita.
- **components/tasks/TaskHubBoard** / **TaskHubCard** — vista Kanban, reusa el `KanbanBoard<T>` genérico de `entity-views/` (mismo que Opportunities/Companies) con columnas sintéticas por fecha (`lib/taskHubDates.ts`'s `bucketForTask`) — Task no tiene un campo de estado propio. Arrastrar a una columna llama `resolveBoardMove(bucket)`: fechas por defecto documentadas ahí, dropear en "Overdue" se rechaza (no se puede forzar manualmente).
- **components/tasks/NewTaskModal** / **TaskDetailModal** — alta (con el picker de entidad) y detalle/edición (meta de solo lectura Created/Due/Completed/Meet + el mismo `TaskForm`), ambos sobre `Modal.tsx`.
- **lib/taskHubDates.ts** — `formatHubDate`/`isOverdue`/`bucketForTask`/`resolveBoardMove`. `toLocalCalendarDate` (interna) normaliza un `dueDate` date-only (medianoche UTC) a fecha local antes de comparar/formatear — sin esto, una fecha date-only se muestra/computa un día antes para cualquier timezone al oeste de UTC (mismo bug que `OverviewPage.tsx`'s `taskDueDateKey` ya resuelve para el calendario).
