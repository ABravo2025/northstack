...
Cada vez que se realice un push a staging o produccion de un cambio visible para el usuario (feature nueva, rediseño, cambio de comportamiento — no un fix interno/infra invisible), deberas agregar una entrada en `frontend/src/lib/changelog.ts` (`CHANGELOG_ENTRIES`, la más nueva primero) describiéndolo en el mismo lenguaje simple que ya usan las entradas existentes, como parte de esa misma tarea. Esto quedó abandonado ~7 semanas (última entrada real: 23 de julio) mientras se shippeaba Custom Roles, CSV overhaul, Payments, el rediseño de Sales v2, notificaciones in-app, Activity Log y el overhaul mobile/Android sin anunciar nada — no lo repitas.
