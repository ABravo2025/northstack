...
Anuncios a la plataforma (`PlatformAnnouncement`, `docs/general/function-index.md` bajo `src/modules/notifications/`, 2026-09-09 — reemplaza el viejo `frontend/src/lib/changelog.ts` hardcodeado, retirado ese mismo día por quedar 7 semanas sin actualizarse): cada vez que se realice un push a staging o produccion de un cambio visible para el usuario (feature nueva, rediseño, cambio de comportamiento — no un fix interno/infra invisible), deberas publicar un anuncio como parte de esa misma tarea, corriendo `DATABASE_URL="..." npx tsx scripts/publish-announcement.ts --type feature_update --title "..." --summary "..." --body "..."` contra el mismo entorno (staging y/o producción) al que se pusheó el código — en el mismo lenguaje simple que ya usaban las entradas viejas del changelog, escrito para quien usa la app, no como mensaje de commit. Aparece en la campana (sección "What's new"), no hace falta tocar código para esto.
Cambios a Terms of Service / Privacy Policy / Refund Policy (`docs/legal/*.md` + `landing` branch): el ToS §14 (y las secciones equivalentes de Privacy/Refund) prometen textualmente "notified via email and an in-app notification" ante cualquier cambio, sin aviso previo. Cada vez que se modifique alguno de estos documentos, además de actualizar el `.md` y la página pública correspondiente en `landing`, deberas publicar el anuncio con `--type policy_change --policyType terms_of_service|privacy_policy|refund_policy` (dispara el email a todo usuario activo automáticamente, no hace falta un paso aparte) — esto no es opcional ni se puede posponer, es cumplir una obligación que el documento ya firma con cada usuario.
Android (Capacitor, `frontend/android/`, workflow `.github/workflows/android-build.yml`, 2026-09-09): cada vez que se promueva un cambio a producción (push a `main`), deberás también disparar la corrida del build de Android como una corrida de fondo, no bloqueante — el workflow hoy se dispara solo en pushes a `main` que tocan `frontend/**` o el propio archivo del workflow; si el cambio promovido no toca esos paths, disparalo igual manualmente (`workflow_dispatch` vía `gh workflow run` o desde la pestaña Actions) para que el APK nativo nunca quede desactualizado respecto a la web. No hace falta esperar a que termine para dar la tarea por cerrada.
Help Center (`/guide` y `/help`, `frontend/src/pages/GuidePage.tsx` y `HelpPage.tsx`, 2026-09-09): cada vez que se implemente o modifique una feature que cambie lo que el usuario ve o puede hacer, actualizá estas dos páginas en la misma tarea para que reflejen el estado real de la app — no quedan para una ronda aparte de "documentación" después. A diferencia del resto de cambios de UI (ver el punto de arriba sobre esperar el visto bueno de Alejandro antes de pushear un cambio visual), esta actualización específica se pushea directo a `staging` y a producción sin esperar esa revisión — excepción explícita pedida por Alejandro, para que esta documentación nunca quede desincronizada del código real.
