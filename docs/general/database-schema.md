...
- **Datos de la empresa (2026-09-25)**: `Tenant.legalName`/`address`/`phone`/`website` (String?) + `logoData` (Bytes?, PNG/JPEG ≤512 KB, achicado a ≤512 px en el navegador) / `logoMimeType` / `logoUpdatedAt` — todos nullable, aditivos, sin backfill. Se editan en Settings → Company (`tenantProfileService.ts`), junto con `companySize`/`industry`/`country` que antes solo se cargaban en el signup. El logo vive en la DB a propósito (no hay file store en el proyecto); `logoData` nunca viaja en `TENANT_SUMMARY_SELECT`, se sirve aparte por `GET /api/public/tenant-logo/:tenantId`. **Sin ID fiscal**: lo prohíbe la privacy policy.
...
(Appearance en Settings — renombrado a "Company profile" el 2026-09-25). `settingsSections.tsx` (compartido por el tile grid y el nav lateral)
