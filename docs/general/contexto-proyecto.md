...
- **2026-09-25**: Settings → Company profile (datos de la empresa + logo; tema movido a Profile) en
  producción — ver `tareas-desarrollo.md` y QA-101. La descripción de Settings más abajo ("Appearance")
  queda histórica.
...
### Cierre de Tier 1 y Tier 2 del backlog de beta-readiness (2026-07-21 a 2026-07-23)

Resumen de alto nivel — el detalle día a día completo vive en `docs/tareas/semana-2026-07-21.md`. Sesión de planning del 2026-07-21 definió las "Prioridades (tiers)" que hoy encabezan `docs/tareas-desarrollo.md`: Tier 1 (cerrar beta-readiness), Tier 2 (data models chicos), Tier 3 (rediseño de Clients), Tier 4 (iniciativas grandes), Tier 5 (cola larga). Se ejecutaron, en orden, un brief de seguridad de 4 bloques (IDOR/mass assignment, rate limiting, Helmet, expiración de sesiones deslizante, checkbox de ToS/Privacy, emails de eventos accionables, canal de feedback, mensaje de agradecimiento + honeypot en Public Forms), los 5 ítems de UX del recorrido de interfaz, resize/reorder/hide-show de columnas de tabla (pedido nuevo del usuario, no en el backlog original), rediseño visual de Employees/Clients/Company Users contra un mockup ya aprobado (avatares, chips de status/rol), seed de datos + onboarding checklist, FAQ, changelog in-app, reportes básicos (como script CLI, no UI — evita adelantar el panel de admin de plataforma), import/export CSV genérico, y los campos nuevos de Employee (job title como catálogo, compensación con visibilidad restringida, fechas, contract URL, personal email) migrando `department` de texto libre a `FieldCatalogDefinition` (mecanismo nuevo, compartido con Job Title, deliberadamente **no** unificado con `StatusDefinition`). Con esto, Tier 1 quedó con todos sus ítems de desarrollo completos (falta solo la revisión manual del usuario en navegador) y Tier 2 quedó 5 de 5.

### Backend dividido en routers por dominio (2026-07-27)

A pedido explícito del usuario ("quiero que separes es app.ts en routers mas chicos, ya que la posibilidad de algun bug es real. usa tecnicas profesionales de programacion y standares"): `src/app.ts` había llegado a 1901 líneas con todas las rutas de la API en un solo archivo. Se extrajo a `src/lib/asyncRouter.ts` (factory `createAsyncRouter()`, mismo wrapper de captura de errores async que ya envolvía cada `app.get/post/...`) y `src/lib/httpAuth.ts` (`validateSession`/`authenticateUser`/helpers), y las rutas se movieron verbatim a un router por dominio bajo `src/routes/`: `auth`, `tenants`, `employees`, `catalogs`, `timeOff`, `views`, `onboarding`, `clients`, `publicForms`, `public`, `feedback` (más `companies`/`contacts`/`pipelines`/`opportunities`, agregados después como parte del rediseño de Clients). `app.ts` quedó en ~60 líneas — solo configuración, montaje de routers, y el error handler final.

### Rediseño de Clients — Company/Contact/Pipeline/Opportunity, 11 unidades (2026-07-27)

El usuario dejó el spec conversacional completo de este rediseño en `docs/tareas-desarrollo.md` (líneas ~389-411, cerrado en una sesión de spec el mismo 2026-07-27) y pidió ejecutarlo pieza por pieza sin pausar para revisión intermedia ("continua con el resto de los units, hasta que no este todo no reviso") — mismo criterio que Time Off en su momento, pero de mayor escala (11 unidades vs. 7). Cada unidad: build → `npm run build`/`npm test` → verificación real (curl y/o Playwright contra un tenant de prueba) → commit → `git push origin main:staging` (nunca a `main`).

1. **Schema**: `Company`, `Contact`, `Pipeline`, `PipelineStageDefinition`, `Opportunity`, `OpportunityContact`, `OpportunityStageHistory` — nuevos, aditivos, sin tocar `Client`. `EntityType` ganó `company`/`contact`/`opportunity`; `CatalogKind` ganó `leadSource`/`lossReason` (reusando `FieldCatalogDefinition` en vez de 2 tablas nuevas). Pieza propia no pedida explícitamente por el spec: `PipelineStageDefinition.outcome` (`open`/`won`/`lost`) — necesaria porque los nombres de stage son renombrables, confirmada con el usuario antes de construir junto con 2 preguntas más (gap de Churned sin trigger de Contract, aceptado tal cual; acceso a la DB de staging, resuelto con el usuario pasando el `STAGING_DATABASE_URL` directamente).
2. **Company** (backend + frontend): CRUD completo, `statusId` nunca editable a mano (deriva de eventos de negocio), tabla full-screen sin Kanban/SavedView (fuera de alcance a propósito).
3. **Contact** (backend + frontend): `companyId` opcional, `leadStatus`/`leadSourceId` solo relevantes para leads de Forms. Bug real encontrado y corregido de paso: `VALID_CATALOG_KINDS` en `src/routes/catalogs.ts` nunca se había actualizado cuando se extendió el enum `CatalogKind` en la Unidad 1 — `GET /api/field-catalog?kind=leadSource` daba 400.
4. **Pipeline/PipelineStageDefinition** (backend + frontend): catálogo configurable por tenant, sembrado inicial "Leads"/"Clientes" con 4 stages cada uno. Bug real encontrado: el sembrado inicial hacía 10 `create()` secuenciales dentro de la misma transacción que el sembrado de `StatusDefinition`, superando el timeout default de 5000ms de Prisma (`P2028`) y rompiendo el registro de tenants nuevos — corregido con `createMany` (2 llamadas en vez de 10 creates) y subiendo el timeout de la transacción a 15000ms.
5. **Opportunity/OpportunityContact/OpportunityStageHistory** (backend + frontend): la pieza central — `maybeAdvanceCompanyToCustomer` dispara `Company.statusId → Customer` cuando un Opportunity entra a un stage `outcome: won` (verificado end-to-end: creado, movido a Won, confirmado por query directa que el status de la Company cambió). Kanban por stage, un tab por Pipeline activo + tab "Archivados". Bug real de React StrictMode encontrado: el efecto que fijaba el tab inicial se ejecutaba 2 veces (double-invoke de desarrollo) y la segunda resolución podía pisar la selección de tab que el usuario ya había hecho a mano — corregido con un `useRef` guard (un chequeo de estado no alcanza, porque el closure del efecto congela el estado inicial en ambas invocaciones).
6. **Form rename + matching**: `PublicForm` no se renombró de tabla (riesgo de un rename real vía `db push` sin tooling de migraciones, sin beneficio funcional) pero ganó `accessMode` (`public`/`internal`, `internal` falla cerrado en la ruta anónima) y `pipelineId`. Un submit de tipo `contact` matchea/crea Company por dominio de email (`matchOrCreateCompanyForContact`, reusa `GENERIC_EMAIL_DOMAINS` del validador de dominio duplicado del Sign Up) y opcionalmente crea un Opportunity en el primer stage del Pipeline configurado.
7. **Migración de datos**: `scripts/backfill-clients-to-companies-contacts.ts`, idempotente — dedupea `Client.company` (texto libre) en una `Company` por nombre normalizado por tenant, y un `Contact` por `Client` (preservando `createdAt` original). Bug real encontrado y corregido de paso: **ningún tenant creado antes de este rediseño tenía filas `StatusDefinition` de `entityType: 'company'`** (el sembrado solo corre al crear un tenant nuevo) — sin eso, `Company` no se podía crear para *ningún* tenant preexistente, no solo los que tenían `Client`. El script las siembra retroactivamente. Corrido y verificado en `staging` (21 Companies/21 Contacts desde 21 `Client`, re-corrida confirmó idempotencia — 0 creados, todo reusado/salteado). **No corrido contra producción todavía** — a diferencia del resto de las unidades (código), esta es una operación real sobre datos de tenants existentes, así que sigue el mismo criterio de esperar revisión del usuario antes de tocar producción.

**Corrección de proceso en medio de la sesión (2026-07-27)**: la Unidad 1 (schema) se pusheó por error tanto a `staging` como a producción en el mismo turno, autoverificada por Claude (build/tests/queries directas) en vez de esperar que el usuario mismo revisara `staging` primero — el gate de staging-first llevaba vigente desde ese mismo día (ver `docs/tareas-desarrollo.md`, nota del 2026-07-24) pero se interpretó mal el criterio de "verificar". El usuario corrigió explícitamente: *"pero tiraste todo a produccion? no lo mandes a produccion sin antes que lo revise en sagin"*. Desde ese momento (Unidades 2 a 11), cada pieza de código quedó **exclusivamente** en `staging` hasta nueva instrucción — sin excepciones, incluyendo cambios que se sentían de bajo riesgo. Actualizado en memoria persistente para que la regla se mantenga entre sesiones.

**Estado al cierre de esta ronda**: 10 de 11 unidades completas y en `staging` (schema + Company + Contact + Pipeline + Opportunity + Form-matching), la 11ª (migración de datos) construida y verificada en `staging` pero bloqueada en producción hasta revisión del usuario. Ninguna unidad de código (más allá del schema aditivo de la Unidad 1, que sí llegó a producción bajo la excepción ya establecida para pushes de schema puramente aditivos) fue promovida a `main`/producción todavía — el corte del módulo `Client` legado (ocultarlo/borrarlo una vez migrados los datos) queda como una unidad futura separada, deliberadamente no construida en esta ronda.

**Seguimiento, 2026-07-28** (detalle completo en `docs/tareas/semana-2026-07-21.md`): el usuario probó el rediseño en `staging` y encontró 2 cosas. Primero, un reporte de "el botón de Pipelines no abre nada" que resultó no ser un bug de código (reproducido localmente contra la base de `staging` sin problema) — se resolvió en una segunda vuelta en que el usuario aclaró que el problema real era de UX: crear un Pipeline exigía guardarlo vacío y recién después agregar stages una por una. Corregido: "New Pipeline" abre ahora un `SlideOver` (nombre + stages agregables en el mismo formulario, mismo patrón que el resto de la app), pusheado a `staging`. Segundo, un gap real: no hay forma de vincular un Contact a una Opportunity al crearla (solo se puede después, editándola) — el usuario pidió explícitamente pausar y hablar con el PM antes de definir cómo resolverlo, así que quedó documentado como ítem abierto sin construir.

### Módulo de Tasks/Notes, unificación de los 4 paneles de detalle y rediseño (2026-07-29 a 2026-07-30)

Sesión larga, continua (cruzó medianoche). Detalle completo, con timestamps y verificación de cada pieza, en `docs/tareas-desarrollo.md` — este resumen es la versión "para orientarse rápido". **Todo lo de esta sección está pusheado a `staging` únicamente (varios commits, el último `306e590`) — nada llegó a `main`/producción todavía**, a la espera de que el usuario revise `staging` en el navegador y dé el visto bueno, según el gate de staging-first vigente desde el 2026-07-27.

**1. Módulo de Tasks** (genérico, cross-entidad — Employee/Company/Contact/Opportunity, no Client): mismo patrón polimórfico `tenantId`+`entityType`+`entityId` que `CustomFieldValue`. Título, descripción, asignado, fecha límite, estado completado. Permisos abiertos a cualquier rol del tenant (decisión confirmada — es una checklist operativa compartida, no dato sensible; revisar cuando exista permisología custom en Tier 5). Se manifiesta en 3 lugares: tab "Tasks" en el panel de detalle de cada entidad, widget "My tasks" en `/overview`, y entradas en el calendario de `/overview`.

**2. Módulo de Notes** (mismo mecanismo que Tasks, sin asignado/fecha/estado — es un registro, no un to-do): título + descripción, con un renderer propio chico de `**bold**`/`*italic*` (regex a elementos React reales, sin `dangerouslySetInnerHTML`, sin librería nueva — no se justificaba para lo que pedía el spec). Los campos se llamaban `header`/`body` hasta que el usuario pidió que coincidieran con Task (`title`/`description`) — migrado con el patrón seguro habitual, preservando la única Note real que el usuario ya había creado probando la app en `staging`.

**3. Checkpoint D — bugs/regresiones (10 ítems)**: encontrados por el usuario probando Tasks contra la app real. Incluye un bug real no relacionado con Tasks que se coló durante el rediseño visual del 07-29 (el modal de confirmación al archivar un Pipeline se había perdido) y varios ajustes de consistencia visual (color de botón en estados vacíos, selector de Pipeline al componente estándar). Detalle ítem por ítem en `docs/tareas-desarrollo.md`.

**4. Checkpoint E — features confirmadas (4 ítems)**: `Company.size` pasó de texto libre a catálogo configurable (mismo mecanismo que Department/Job Title); crear una Company ahora exige un Contact (nuevo o existente) en el mismo paso — encontrado al implementar que `ContactDetailModal.tsx` ya creaba una Company "al vuelo" en un flujo distinto, unificado en el mismo endpoint; crear un Contact ganó un flujo de asignación a Company con selector con buscador (`SearchableSelect.tsx`, componente nuevo genérico, sin librería externa); y el módulo de Notes completo.

**5. Checkpoint F — unificación estructural**: `EmployeeOverviewPanel.tsx` (el último de los 4 que quedaba con tabs + botón "Edit") migró al mismo patrón autosave que ya tenían Company/Contact/Opportunity — se retiraron ~230 líneas de un form de edición separado que quedó completamente redundante. Excepción confirmada con el usuario: a diferencia de Company/Contact (que hoy no tienen forma de renombrarse desde el panel de detalle — un gap real preexistente, no tocado), Employee sí mantiene nombre/email editables in-place, para no perder una capacidad que ya existía. Se evaluó construir un componente de detalle único para los 4 — se descartó por tener variación real de campos/comportamiento entre entidades; lo único genuinamente idéntico (`Field`, el wrapper de cada campo) se extrajo a un archivo compartido.

**6. Rediseño visual de los 4 paneles de detalle** (pedido del usuario después de probar Checkpoint F en el navegador, con un mockup anotado): pasaron de un popup angosto centrado (460px) a **70vw × 70vh, 2 columnas** — izquierda un grid de 2 columnas de campos (antes 1 columna sola, mucho espacio desperdiciado; una clase `overview-field-full`/prop `full` en `Field` para las secciones que sí necesitan el ancho completo, como listas de registros vinculados), derecha una columna con tabs **Notes / Tasks / Activity** (`DetailSidebar.tsx`, componente nuevo, literal el mismo en los 4 paneles). El compose de Notes/Task pasó de un popover que se abría al hacer click a un formulario **siempre expandido** en la columna derecha — simplificó bastante el código (ya no hace falta la lógica de anchor/posicionamiento del popover). Activity entra como tab desde ya (revierte una decisión del 07-29 de que fuera un panel lateral sin tab) pero sigue siendo un placeholder — no hay sistema de auditoría real detrás todavía (Tier 5).

**7. Bugs reales encontrados por el usuario probando en el navegador, los 3 corregidos**:
- El header del panel de detalle pasó de centrado/apilado a una sola línea (Avatar — Nombre — Email/Industria — Status), y el compose de Notes/Task ganó contraste real (fondo blanco/`gray-900` + borde, contra una columna derecha con tinte gris) porque antes ambos eran del mismo color y el recuadro no se notaba. De paso se encontró que la clase `.nv-field` nunca había estilizado `textarea` (solo `input`/`select`) — afectaba a cualquier textarea de ese patrón en toda la app, no solo Notes.
- **Guardar un campo (ej. Personal Email) no se reflejaba al salir del panel**: causa real, `save()` en los 4 paneles nunca avisaba al componente padre que algo había cambiado — el dato se guardaba bien en el backend, pero la lista en memoria del padre (`employees`/`companies`/etc.) nunca se refrescaba. Corregido primero con un refresh en background silencioso (sin flash de loading, ya lo tenía Opportunity, faltaba en los otros 3), y **después mejorado a un patch instantáneo de la fila** usando la respuesta que ya devuelve el propio `PATCH` (sin round-trip extra) — el refresh en background se mantiene corriendo en paralelo para los campos que son FK (Department, Status, Company, Stage), porque la respuesta del `PATCH` no siempre trae el objeto relacionado completo (ej. cambiar `stageId` no devuelve el `stage.name` nuevo).
