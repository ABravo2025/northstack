  ...
  return (
    <>
      <button
        ...
        onClick={handleOpen}
        aria-label={totalUnread > 0 ? t('notifications.ariaUnread', { count: totalUnread }) : t('notifications.title')}
      ...
      <Popover open={open} onClose={() => setOpen(false)} anchorRef={triggerRef} align="right" width={320}>
        <div className="flex items-center justify-between">
          <div className="color-picker-section-label">{t('notifications.title')}</div>
          {unreadCount > 0 && (
        ...
        <div className="notification-list mt-2">
          {loaded && notifications.length === 0 && <p className="notification-empty">{t('notifications.empty')}</p>}
          {notifications.map((n) => (
            <button
              key={n.id}
              type="button"
